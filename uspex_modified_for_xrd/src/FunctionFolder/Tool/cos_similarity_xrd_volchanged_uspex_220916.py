...
if __name__ == "__main__":
    ...
    two_theta_range=(xrd_2theta_min,xrd_2theta_max)    ### 220913 changed
    d_two_theta = d_2theta ;     sigma = xrd_sigma   # 0.5     # 220913 changed
    two_thetas = np.arange(xrd_2theta_min,xrd_2theta_max+0.5*d_two_theta,d_two_theta)
#    print ('two_thetas : ',len(two_thetas))
#    sigma = xrd_sigma   # 0.5     # 220913 changed
    method = "XRD"
    if bool_gsas == 'True':
       chi1 = cif.CifParser(sys.argv[1]).get_structures(primitive=False)[0]
       spectra1 = Spectra(chi1, two_thetas, method='XRD',use_gsas=True)
       spectra1_smeared = gauss_filter2(spectra1.f_result,d_two_theta,sigma)
    else: # bool_gsas == 'False':
       chi1 = cif.CifParser(sys.argv[1]).get_structures(primitive=False)[0]
       spectra1 = Spectra(chi1, two_thetas, method='XRD')
       spectra1_smeared = gauss_filter2(spectra1.f_result,d_two_theta,sigma) #,target_xrd_intensity,d_2theta,sigma)
#    data = np.column_stack((spectra1.two_thetas,spectra1.f_result))    # for uspex 220916
#    np.savetxt(sys.argv[1].split('.cif')[0]+'_xrd.csv',data,delimiter=',')    # for uspex 220916
#    data = np.column_stack((spectra1.two_thetas,spectra1_smeared))   # for uspex 220916
#    np.savetxt(sys.argv[1].split('.cif')[0]+'_smeared_xrd.csv',data,delimiter=',')       # for uspex 220916
...
#    np.savetxt(sys.argv[2].split('.cif')[0]+'_volchanged_smeared_xrd.csv',data2,delimiter=',')   # for uspex 220916
    ...
    o = open('cos_similarity_xrd_volchanged.csv','w')
