...
### Modified for GSAS by Junpei Oba ###
########################################
# 210419 #

class Spectra():
    def __init__(self,chi_real, two_theta_range, d_two_theta, xray_wavelength,method="XRD", use_gsas=False):
        '''
        Parameters:
        ----------
        ...
        if use_gsas==False:
            ...
            two_thetas = np.arange( two_theta_range[0], two_theta_range[1]+0.5*d_two_theta, d_two_theta)
...
#        /gsas
            ...
            PID = os.getpid()
            os.makedirs('./gsas-tmp/', exist_ok=True)
            TMP_DIR = './gsas-tmp/' +str(PID) + '_'  ## temporary folder to store sim.gpx, sim.cif, etc.
            DATA_DIR = './' ## path where 'INST_XRY.PRM' file exists.
            ...
            hist1 = gpx.add_simulated_powder_histogram("simulation",
                                      ...
                                      two_theta_range[0],two_theta_range[1],d_two_theta,
                                      phases=gpx.phases())
